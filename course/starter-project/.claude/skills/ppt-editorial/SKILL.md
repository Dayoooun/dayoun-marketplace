...
> **섹션 composition preset:** 모든 HTML layout은 `compositionPreset`을 사용한다. `cover` 3종(`hero-right|hero-left|stacked`), `table` 3종(`evidence-wide|entity-led|outcome-led`), `kpi` 3종(`lead-left|lead-right|interpretation-left`), `bars` 3종(`plot-left|plot-right|summary-bottom`), `overview` 3종(`metadata-rail|donut-stage|distribution-stage`), `process` 4종(`horizontal-rail|vertical-ledger|vertical-focus|vertical-visual`), `modules` 4종(`feature-left|feature-right|feature-top|ledger`), `roadmap` 3종(`staircase-rise|staircase-fall|phase-ledger`), `break` 3종(`time-left|time-right|return-band`), `network` 3종(`horizontal-flow|vertical-flow|hub-spoke`), `image` 3종(`copy-left|copy-right|visual-top`)이다. 같은 layout이 반복되면 같은 preset을 연속 사용하지 않는다. 옛 `modulePreset`은 사용하지 않는다.
> **Process + Codex image:** `vertical-visual`은 좌측 HTML rail과 우측 Codex 이미지 필드를 결합한다. `asset.role`은 `3d-scene` 또는 `material-illustration`이어야 하며, 이미지에는 글자·숫자·라벨·화살표를 굽지 않는다. process 문구·번호·rail은 HTML만 소유한다. 우측이 비는 수직 process 페이지에는 `vertical-ledger`를 억지로 늘리지 말고 이 preset을 선택한다.
...
> **디자인 루틴 수:** 스타일 프로파일 8종, 기본 통합 프로파일 내부 변형 3종, HTML 역할 레이아웃 11종, 섹션 composition preset 35종, 검증 시나리오 14종, few-shot 15종이다. 프로파일은 `toss-data-unified`, `modern-flat`, `data-report-editorial`, `paper-serif`, `swiss-grid`, `warm-editorial`, `technical-blueprint`, `photo-documentary`다. 각 프로파일은 HTML preset class와 Codex prompt를 함께 바꾼다.
