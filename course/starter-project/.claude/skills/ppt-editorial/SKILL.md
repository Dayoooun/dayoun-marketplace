...
> **표지 디자인:** 표지는 `visualRole: "cover"`와 `layout: "cover"`를 쓴다. 제목은 2줄 이내, 정확한 제목·부제·메타데이터는 HTML로 유지한다. 우측 62%에는 고품질 3D 또는 사진 자산 하나만 두고 자산은 프레임 폭의 65~80%를 차지하게 한다. 좌측 34%에는 한 문장 포지셔닝과 대상·형식·버전 2~3개만 둔다. 중앙에 제목만 띄우기, 작은 아이콘 콜라주, 본문용 카드 그리드, 푸터 크롬, 의미 없는 장식은 금지한다.
> **휴식 안내:** 작은 타이머 카드 하나와 멀리 떨어진 하단 문장으로 여백을 메우지 않는다. 좌측 40%에는 시간·구간·휴식 목적을 한 패널로 묶고, 우측 56%에는 복귀 안내·재개 활동 2~3개를 배치한다. 본문은 29~84%를 사용하며 하단 25%가 비면 실패다. `visualRole: "break"`와 `layout: "break"`를 쓴다.
