...
> **디자인 루틴 수:** 스타일 프로파일 8종, 기본 통합 프로파일 내부 변형 3종, HTML 역할 레이아웃 11종, 섹션 composition preset 34종, 검증 시나리오 14종, few-shot 14종이다. 프로파일은 `toss-data-unified`, `modern-flat`, `data-report-editorial`, `paper-serif`, `swiss-grid`, `warm-editorial`, `technical-blueprint`, `photo-documentary`다. 각 프로파일은 HTML preset class와 Codex prompt를 함께 바꾼다.
