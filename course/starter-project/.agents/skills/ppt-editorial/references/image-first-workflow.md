...
## 1.0 delivery contract

The stages below choose and refine the design; they do not authorize delivery. Before the first render, build `visible-text-manifest.json` from the canonical payload and ordered deck briefs. Every visible occurrence needs a stable `textId`, occurrence number, normalized region, token kind, and each relation needs a direction and geometry.

Store payload, briefs, derived manifest, and approval envelope with `approved_inputs.py build --renderer-version image-first-v1`. Rendering receives only the returned approval digest and content-addressed store. Immediately after rendering, use `approved_inputs.py record-render` to bind ordered slide identities, filenames, and PNG digests; assembly requires that receipt digest. A payload, brief, manifest, mode, renderer, normalizer, mapper, scene config, referenced asset, prompt, or rendered pixel change makes the approval/receipt stale.

Rasterize the delivered PPTX and PDF, run locked OCR, and call `ocr/validate_visible_text.py` with the approval digest/store. Each OCR report must name the exact source artifact digest, page count, and every page raster digest. Missing or extra text, one-glyph differences, swapped or geometrically wrong relations, non-finite/low confidence, alternatives, clipping, tofu, ambiguous regions, stale OCR, PPTX/PDF disagreement, or missing toolchain provenance is `AUTO_BLOCK`. Fact, structure, contract, mode automation, and external visual checks must all PASS; none overrides another.
