...
| 주장 출처 | 제목을 제외한 모든 주장 문단에 `[E/U/H/P### | 짧은 출처·상태]` 표지가 보이고 `근거목록.csv`의 유효 ID·상세 출처와 일치하여 추적률이 100%인가 | PASS/BLOCK |
| 표현 | 과장, 모호한 수식어, 지나치게 긴 문장을 줄였는가 | PASS/WARN/BLOCK |
| 문체·문단 | 종결 문체가 같고 한 문단에 하나의 핵심을 담았는가 | PASS/WARN/BLOCK |
| 서식 | 공식 양식과 번호 체계를 지키고, 장문 답변이 `o 핵심항목`·`- 세부내용` 개조식과 목록 들여쓰기·실제 내어쓰기를 사용하며 제목·세부 굵기가 구분되는가 | PASS/WARN/BLOCK |
