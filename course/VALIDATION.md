...
- 일곱 개 핵심 SKILL.md와 한 개 선택 SKILL.md에 특정 기관명·프로그램명·전용 평가영역이 없음을 자동 검사
...
- 일곱 개 핵심 스킬과 한 개 선택 스킬 공식 검증기: 모두 통과
- Codex·Claude Code·Antigravity용 매니페스트 이름과 버전 일치
- 원본 스킬과 스타터의 `.agents/skills`, `.claude/skills`: 68개 파일씩 바이트 단위 일치
- 프로젝트 생성기와 스타터 프로젝트: 25개 템플릿 일치
- 공개 마켓플레이스 자동 테스트: Python 3.13과 CI 기준 Python 3.11에서 각각 29개 통과
...
- 견적서 CLI smoke: 통과
- 프로필·이력서 CLI smoke: 통과
- 공문·안내문·게시용 CLI smoke: 통과
- 이미지 backend 미설치 degradation: placeholder 유지, 통과
- 오프라인 Chromium A4 화면·PDF 검사: 세 모듈 통과, 원격 stylesheet 0개, 한글 누락 상자 없음, 각 PDF 1페이지
- 개인정보 형태 검사와 가상 example allowlist: 통과
- 생성 HTML·이미지의 저장소 미포함 검사: 통과
- 수동 원본 대비 sanitization 검토:
  - `scratchpad/donglae/gpts/01-견적서/knowledge/quote_render.py`: PASS
  - `scratchpad/donglae/gpts/02-프로필/knowledge/profile_render.py`: PASS
  - `scratchpad/donglae/gpts/03-공문안내문/knowledge/notice_render.py`: PASS
- 수동 검토 기록은 경로와 판정만 남겼으며 확인한 값은 기록하지 않음

## 공개 경로

- 공개 원본: `https://github.com/Dayoooun/dayoun-marketplace`
- 웹 과정: `/courses/practice/12-modoo-startup-plan`
- 공개 ZIP: `https://github.com/Dayoooun/dayoun-marketplace/releases/download/v0.9.0/dayoun-marketplace-v0.9.0.zip`
