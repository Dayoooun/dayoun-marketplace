...
SKILL = ROOT / "plugins" / "business-plan-writer" / "skills" / "create-business-documents"
...
class BusinessDocumentsSkillTests(unittest.TestCase):
    ...
    def test_skill_inventory_and_boundary_statement(self) -> None:
        ...
        self.assertIn("스타터 워크스페이스", router)
        self.assertIn("강의 커리큘럼에는 포함되지 않습니다", router)
    ...
    def test_optional_skill_stays_outside_core_workflow_and_release_is_automated(self) -> None:
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        workflow_section = readme.split("## 공고에서 발표까지 이어지는 10단계", 1)[1].split(
            "## 제공하는 7개 핵심 스킬 + 1개 선택 스킬", 1
        )[0]
        self.assertNotIn("create-business-documents", workflow_section)
        self.assertIn("사업계획서 10단계와 별개로 쓰는 선택 스킬", readme)

        for relative in (
            "plugins/business-plan-writer/plugin.json",
            "plugins/business-plan-writer/.codex-plugin/plugin.json",
            "plugins/business-plan-writer/.claude-plugin/plugin.json",
            ".agents/plugins/marketplace.json",
            ".claude-plugin/marketplace.json",
        ):
            payload = json.loads((ROOT / relative).read_text(encoding="utf-8"))
            if relative.endswith("marketplace.json"):
                version = payload["plugins"][0]["version"]
            else:
                version = payload["version"]
            self.assertEqual(version, "0.9.0", relative)
        ...
        self.assertIn('python scripts/build_release_zip.py', release)
        self.assertRegex(release, r"permissions:\s+contents: read")
        self.assertRegex(release, r"release:\s+needs: validate\s+runs-on: ubuntu-latest\s+permissions:\s+contents: write")
        self.assertIn("persist-credentials: false", release)
        self.assertIn('EXPECTED_SKILLS = 8', builder)
        self.assertIn('git\", \"ls-files\"', builder)
