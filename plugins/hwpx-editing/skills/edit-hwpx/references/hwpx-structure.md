...
## 11. ★ 부동 서명·직인 — 앵커 문단이 제목 문단(idx 0)이어야 한다

### 증상

수진기업 "성 명" 칸이 세로로 늘어나 표 전체가 비뜼어진다.
셀 높이(`cellSz`)는 정상인데 렌더만 틀어진다.

### 원인 — `treatAsChar` 가 아니라 앵커 문단

서명이 `treatAsChar="0"`(부동)이고 표 밖에 있어도, `vertRelTo="PARA"` 의
기준이 되는 **앵커 문단이 표 안 문단**이면 그 셀을 밀어낸다.

| 문서 | 앵커 문단 idx | vertOffset | 판정 |
|---|---|---|---|
| 헤어리뉴·큰당구장·플로렌·신디 (5건) | **0** | 1765~8000 | 정상 |
| 쇼미더브이알 | 22 | 0 | 표가 늘어남 |
| 이음하라 | 42~92 | 0~1287 | 동일 증상 가능 |

### 정상 구조 (신디필라테스 기준)

제목 문단(idx 0)의 **한 `<hp:run>` 안에** 직인과 서명이 **연속으로** 놓인다.

```xml
<hp:p id="..." paraPrIDRef="22">
  <hp:run charPrIDRef="...">
    <hp:pic .../>   <!-- image1 컨설턴트 직인 -->
    <hp:pic .../>   <!-- image2 대표자 서명 — 바로 뒤에 붙는다 -->
    <hp:t>l</hp:t>
  </hp:run>
</hp:p>
```
...
```bash
# 진단
python scripts/hwpx_sign.py <파일.hwpx>

# 교정 — 정상 문서를 레퍼런스로 주면 pos 를 그대로 승계한다 (권장)
python scripts/hwpx_sign.py <대상.hwpx> --fix --ref <정상.hwpx>
```

교정은 **`<hp:pic>` 단위로만** 이동한다 — 직인 pic 바로 뒤에 붙인다.
run·문단은 건드리지 않는다.

### ⚠️ 가드가 필수인 이유

pic 을 임의 위치에 붙였다가 **표 5개가 통째로 사라졌다**(2026-08-27 실측).
`ET.fromstring` 문법 검사는 통과하므로 구조 불변을 따로 확인해야 한다.

`hwpx_sign.py` 는 교정 후 **`<hp:tbl>`·`<hp:tc>`·`<hp:p>` 개수**를 비교해
하나라도 변하면 교정을 취소한다.

### 검증

`hwpx_verify.py` 가 앵커 문단을 게이트로 검사한다(비정상이면 FAIL).
