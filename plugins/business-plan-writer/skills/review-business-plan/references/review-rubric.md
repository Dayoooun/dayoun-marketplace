...
| 주장 출처 | 초안·승인본의 `[E/U/H/P### | 짧은 출처·상태]`가 `근거목록.csv`와 일치하고, 최종 HWPX가 `(출처명, YYYY)` 형식으로 렌더되어 추적률이 100%인가 | PASS/BLOCK |
| 표현 | 과장, 모호한 수식어, 지나치게 긴 문장을 줄였는가 | PASS/WARN/BLOCK |
| 문체·문단 | 종결 문체가 같고 한 문단에 하나의 핵심을 담았는가 | PASS/WARN/BLOCK |
| 서식 | 공식 양식의 번호 체계를 지키고, 식별 라벨을 제외한 답변이 `o 핵심항목`(대항목)·`- 핵심항목`·`· 세부내용` 개조식이며 각 단계에 실제 내어쓰기·들여쓰기가 적용됐는가 | PASS/BLOCK |
