---
name: ppt-editorial
description: "16:9 발표 덱 제작. **씬(배경 일러스트)만** 이미지 생성 모델이 그리고 헤드라인·서브·라벨·쪽번호는 코드가 렌더하므로 한글이 깨지지 않고 수정이 초 단위다. 도메인 한 줄로 팔레트·서체·톤이 정해지고(9종) 구도 10종을 골라 배치한다. PDF와 편집 가능한 PPTX를 낸다. 제안서·결과보고·IR·강의자료에 쓴다. 슬라이드 전체를 한 장 이미지로 만들고 싶으면 이 스킬이 아니라 image-first 계열을 쓴다."
---

# 슬라이드 덱 하네스 — 씬 덱 방식


> **이 스킬을 고르는 기준** — 텍스트가 정확해야 하고(한글 깨짐 불가), 나중에 문구를
> 고칠 일이 있으면 이 스킬이다. 씬은 모델이 그리고 글자는 코드가 얹는다.
> 반대로 "한 장을 통째로 그림처럼" 뽑고 텍스트 정확도를 양보할 수 있으면 image-first 계열이 맞다.
>
> 두 방식 모두 같은 이미지 생성 엔진을 쓴다. 차이는 **모델에게 무엇을 그리게 하느냐**다.

> ⚠️ 스킬명 `ppt-editorial`은 초기 이름이 남은 것이다. 지금은 씬 덱 방식 하나만 담는다.
